...
const CarbonAnalysisDetails = () => {
  ...
  const getChartData =
    projectSite &&
    projectSite[0].sites.filter(
      (i) => i.site_name === siteDropDownDetails.site_name,
    );
  const filterTransitionData = getChartData
    ? siteDropDownDetails?.lulc_transition_piechart
    : "";
  const filterTimeSeriesData = getChartData
    ? siteDropDownDetails?.lulc_timeseries
    : "";
  const filterWaterData = getChartData
    ? siteDropDownDetails?.surface_water_transition
    : "";
  const filterSoilData = getChartData ? siteDropDownDetails?.soc : "";
  const filterSoilPropertyData = getChartData
    ? siteDropDownDetails?.soil_properties
    : "";
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      ...
      if (
        JSON.stringify(updatedDetails) !== JSON.stringify(siteDropDownDetails)
      ) {
  ...
  const soilPropertyDataCategories = Object.keys(
    filterSoilPropertyData.sand,
  ).filter((key) => key.endsWith("cm"));
  ...
  const series = Object.entries(filterSoilPropertyData).map(([key, data]) => ({
    name: key,
    data: soilPropertyDataCategories.map(
      (category) => (data as Record<string, number>)[category],
    ),
  }));
  ...
  const options1 = {
    ...
    tooltip: {
      formatter: function (
        ...
        const seriesName =
          this.series.name.charAt(0).toUpperCase() + this.series.name.slice(1); // Series name
  ...
  useEffect(() => {
    ...
    const categories = Object.keys(filterTimeSeriesData).filter(
  ...
  const carbonAnalysisData = [
    {
      label: "Total Above Ground Biomass",
      value: siteDropDownDetails?.agb + " " + siteDropDownDetails?.agb_unit,
      xs: 4,
      sm: 2,
    },
    {
      label: "Total Mean Above Ground Biomass",
      value:
        siteDropDownDetails?.mean_agb +
        " " +
        siteDropDownDetails?.mean_agb_unit,
      xs: 4,
      sm: 3,
    },
    {
      label: "Total Cstock Above Ground Biomass",
      value:
        siteDropDownDetails?.cstock_agb +
        " " +
        siteDropDownDetails?.cstock_agb_unit,
      xs: 4,
      sm: 3,
    },
    {
      label: "Total Mean Cstock Above Ground Biomass",
      value:
        siteDropDownDetails?.mean_cstock_agb +
        " " +
        siteDropDownDetails?.mean_cstock_agb_unit,
      xs: 4,
      sm: 2,
    },
    {
      label: "Total Below Ground Biomass",
      value: siteDropDownDetails?.bgb + " " + siteDropDownDetails?.bgb_unit,
      xs: 4,
      sm: 2,
    },
    {
      label: "Total Mean Below Ground Biomass",
      value:
        siteDropDownDetails?.mean_bgb +
        " " +
        siteDropDownDetails?.mean_bgb_unit,
      xs: 4,
      sm: 3,
    },
    {
      label: "Total Cstock Below Ground Biomass",
      value:
        siteDropDownDetails?.cstock_bgb +
        " " +
        siteDropDownDetails?.cstock_bgb_unit,
      xs: 4,
      sm: 3,
    },
    {
      label: "Total Mean Cstock Below Ground Biomass",
      value:
        siteDropDownDetails?.mean_cstock_bgb +
        " " +
        siteDropDownDetails?.mean_cstock_bgb_unit,
      xs: 4,
      sm: 2,
    },
  ];
  ...
  return (
    <Box sx={{ mt: 4 }}>
      ...
      <SiteDetailCard
        siteTitle={siteDropDownDetails?.site_name}
        siteDetail={siteDropDownDetails?.type}
      />
      <Box sx={{ mb: 3 }}>
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        <Grid size={{ xs: 12 }}>
          <Card
            sx={{
              boxShadow:
                "0px 0px 5px 0px rgba(0, 0, 0, 0.1019607843) !important",
              borderRadius: "8px",
              backgroundColor: "#fff",
            }}
          >
        ...
        {transformedData.length ? (
          <Grid size={{ xs: 6 }}>
            <ChartComponent
              chartType={"pie"}
              chartTitle={"Land Cover Area Distribution - 2000 (in ha)"}
        ...
        {transformedData.length ? (
          <Grid size={{ xs: 6 }}>
            <ChartComponent
              chartType={"pie"}
              chartTitle={"Land Cover Area Distribution - 2022 (in ha)"}
              data={transformedData.map((item) => ({
                name: item.name,
                value1: item.value2,
              }))}
        ...
        <Grid size={{ xs: 12 }}>
          <ChartComponent
            chartType={"pie"}
            chartTitle={"Surface Water Transition (1984-2021) (in ha)"}
            data={transformedSurfaceWaterData}
            seriesValue1="Area"
          />
        </Grid>
        <Grid size={{ xs: 12 }}>
      ...
      <Box>
        <Typography
          sx={{ fontSize: "24px", fontWeight: 600, color: "#151E15" }}
        >
          {"Biomass & Cstock"}
        </Typography>
      </Box>
